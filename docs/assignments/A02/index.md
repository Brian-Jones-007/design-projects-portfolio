...
* **$\Sigma M_A = 0$**  ➔  **$B_y = -8.333\text{ kN}$** (acting downward)
* **$\Sigma F_y = 0$**  ➔  **$A_y = +8.333\text{ kN}$** (acting upward)
* **$\Sigma F_x = 0$**  ➔  **$A_x = 0.000\text{ kN}$**
...
* **Member AD:** $F_{AD} = +41.67\text{ kN}$ (Tension) — **Peak Tensile Force**
...
* **Allowable Stress:** $\sigma_{allow} = \frac{350\text{ MPa}}{3.5} = \mathbf{100\text{ MPa}}$
* **Minimum Cross-Sectional Area:** $A_{truss} = \frac{41.67\text{ kN}}{100\text{ MPa}} = \mathbf{462.96\text{ mm}^2}$
...
Support pin connections were evaluated under double shear conditions using hardened steel pins ($\tau_y = 1172\text{ MPa}$) with a safety factor $FS_{pin} = 4.0$ based on the maximum shear force $V_{max} = 8.333\text{ kN}$.

* **Allowable Shear Stress:** $\tau_{allow} = \frac{1172\text{ MPa}}{4.0} = \mathbf{293.00\text{ MPa}}$
* **Minimum Shear Area:** $A_{pin} = \frac{8.333\text{ kN}}{2 \times 293.00\text{ MPa}} = \mathbf{14.22\text{ mm}^2}$
* **Minimum Pin Diameter:** $d_{pin} = \mathbf{4.26\text{ mm}}$

Rounding up to the nearest standard commercial component size yields a **$d_{pin} = 5.00\text{ mm}$** diameter pin.
...
Native Creo Mass Properties analysis confirmed a total part volume of 1.477 × 10⁶ mm³ and a total frame mass of **11.59 kg**, closely resembling the theoretical hand-calculated frame mass. Adding four 5.0 mm steel pins (0.015 kg) yields a total system assembly mass of **11.61 kg** (123.30 N).
...
* [Download Truss Frame Creo Part File (`truss_frame.prt`)](truss_frame1.prt.1)
